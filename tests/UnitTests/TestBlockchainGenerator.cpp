...
public:
  ...
  void generate(const AccountPublicAddress& address, Transaction& tx, uint64_t unlockTime = 0)
  {
    ...
    Dogero::decompose_amount_into_digits(this->m_source_amount, 0,
      [&](uint64_t chunk) { destinations.push_back(Dogero::TransactionDestinationEntry(chunk, address)); },
      [&](uint64_t a_dust) { destinations.push_back(Dogero::TransactionDestinationEntry(a_dust, address)); });

    Dogero::constructTransaction(this->m_miners[this->real_source_idx].getAccountKeys(), this->m_sources, destinations, std::vector<uint8_t>(), tx, unlockTime, m_logger);
  }
...
TestBlockchainGenerator::TestBlockchainGenerator(const Dogero::Currency& currency) :
  m_currency(currency),
  generator(currency)
{
...
std::vector<Dogero::Block>& TestBlockchainGenerator::getBlockchain()
{
  std::unique_lock<std::mutex> lock(m_mutex);
  return m_blockchain;
}

std::vector<Dogero::Block> TestBlockchainGenerator::getBlockchainCopy() {
  std::unique_lock<std::mutex> lock(m_mutex);

  std::vector<Dogero::Block> blockchain(m_blockchain);
  return blockchain;
}

bool TestBlockchainGenerator::getTransactionByHash(const Crypto::Hash& hash, Dogero::Transaction& tx, bool checkTxPool)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
const Dogero::AccountBase& TestBlockchainGenerator::getMinerAccount() const {
  std::unique_lock<std::mutex> lock(m_mutex);
  return miner_acc;
}
...
void TestBlockchainGenerator::addGenesisBlock() {
  ...
  m_timestampIndex.add(m_currency.genesisBlock().timestamp, Dogero::get_block_hash(m_currency.genesisBlock()));
...
void TestBlockchainGenerator::addMiningBlock() {
  ...
  Dogero::Block& prev_block = m_blockchain.back();
  ...
  m_timestampIndex.add(block.timestamp, Dogero::get_block_hash(block));
...
void TestBlockchainGenerator::addTxToBlockchain(const Dogero::Transaction& transaction)
{
  std::unique_lock<std::mutex> lock(m_mutex);
  addToBlockchain(transaction);
}

bool TestBlockchainGenerator::getBlockRewardForAddress(const Dogero::AccountPublicAddress& address)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::generateTransactionsInOneBlock(const Dogero::AccountPublicAddress& address, size_t n) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::getSingleOutputTransaction(const Dogero::AccountPublicAddress& address, uint64_t amount) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
void TestBlockchainGenerator::addToBlockchain(const Dogero::Transaction& tx) {
  addToBlockchain(std::vector<Dogero::Transaction> {tx});
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<Dogero::Transaction>& txs) {
  addToBlockchain(txs, miner_acc);
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<Dogero::Transaction>& txs, const Dogero::AccountBase& minerAddress) {
  std::list<Dogero::Transaction> txsToBlock;
  ...
  std::vector<Dogero::Transaction>& new_txs, std::vector<Crypto::Hash>& deleted_tx_ids)
{
  ...
  if (known_block_id != Dogero::get_block_hash(m_blockchain.back())) {
    is_bc_actual = false;
    return;
  }
...
void TestBlockchainGenerator::putTxToPool(const Dogero::Transaction& tx) {
  std::unique_lock<std::mutex> lock(m_mutex);

  Crypto::Hash txHash = Dogero::getObjectHash(tx);
  m_txPool[txHash] = tx;
}

void TestBlockchainGenerator::putTxPoolToBlockchain() {
  std::unique_lock<std::mutex> lock(m_mutex);
  std::vector<Dogero::Transaction> txs;
...
void TestBlockchainGenerator::setMinerAccount(const Dogero::AccountBase& account) {
  miner_acc = account;
}
...
bool TestBlockchainGenerator::getPoolTransactionIdsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<Crypto::Hash>& hashes, uint64_t& transactionsNumberWithinTimestamps) {
  ...
  for (auto i : m_txPool) {
    ...
    hashes.push_back(Dogero::getObjectHash(i.second));
    ++c;
  }
  return true;
}
...
void TestBlockchainGenerator::addTx(const Dogero::Transaction& tx) {
...
bool TestBlockchainGenerator::generateFromBaseTx(const Dogero::AccountBase& address) {
  std::unique_lock<std::mutex> lock(m_mutex);
