...
TransactionBuilder::TransactionBuilder(const Dogero::Currency& currency, uint64_t unlockTime)
  : m_currency(currency), m_version(Dogero::CURRENT_TRANSACTION_VERSION), m_unlockTime(unlockTime), m_txKey(generateKeyPair()) {}
...
TransactionBuilder& TransactionBuilder::setTxKeys(const Dogero::KeyPair& txKeys) {
  m_txKey = txKeys;
  return *this;
}

TransactionBuilder& TransactionBuilder::setInput(const std::vector<Dogero::TransactionSourceEntry>& sources, const Dogero::AccountKeys& senderKeys) {
  m_sources = sources;
...
TransactionBuilder& TransactionBuilder::setOutput(const std::vector<Dogero::TransactionDestinationEntry>& destinations) {
  m_destinations = destinations;
  return *this;
}

TransactionBuilder& TransactionBuilder::addOutput(const Dogero::TransactionDestinationEntry& dest) {
...
Transaction TransactionBuilder::build() const {
  ...
  std::vector<Dogero::KeyPair> contexts;
...
void TransactionBuilder::fillInputs(Transaction& tx, std::vector<Dogero::KeyPair>& contexts) const {
...
void TransactionBuilder::signSources(const Crypto::Hash& prefixHash, const std::vector<Dogero::KeyPair>& contexts, Transaction& tx) const {
