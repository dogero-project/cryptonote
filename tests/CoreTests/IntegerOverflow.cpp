...
namespace
{
  ...
  void append_TransactionSourceEntry(std::vector<Dogero::TransactionSourceEntry>& sources, const Transaction& tx, size_t out_idx)
  {
    Dogero::TransactionSourceEntry se;
    se.amount = tx.outputs[out_idx].amount;
    se.outputs.push_back(std::make_pair(0, boost::get<Dogero::KeyOutput>(tx.outputs[out_idx].target).key));
    se.realOutput = 0;
...
bool gen_uint_overflow_base::check_tx_verification_context(const Dogero::tx_verification_context& tvc, bool tx_added, size_t event_idx, const Dogero::Transaction& /*tx*/)
{
  return m_last_valid_block_event_idx < event_idx ? !tx_added && tvc.m_verifivation_failed : tx_added && !tvc.m_verifivation_failed;
}

bool gen_uint_overflow_base::check_block_verification_context(const Dogero::block_verification_context& bvc, size_t event_idx, const Dogero::Block& /*block*/)
{
...
bool gen_uint_overflow_base::mark_last_valid_block(Dogero::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
...
bool gen_uint_overflow_1::generate(std::vector<test_event_entry>& events) const
{
  ...
  std::list<Dogero::Transaction> txs_1;
