...
namespace Tests {
  ...
  namespace po = boost::program_options;
    ...
    public:
      BaseFunctionalTests(const Dogero::Currency& currency, System::Dispatcher& d, const BaseFunctionalTestsConfig& config) :
          m_dispatcher(d),
          m_currency(currency),
    ...
    protected:
      ...
      bool makeWallet(std::unique_ptr<Dogero::IWalletLegacy> & wallet, std::unique_ptr<Dogero::INode>& node, const std::string& password = "pass");
      bool mineBlocks(TestNode& node, const Dogero::AccountPublicAddress& address, size_t blockCount);
      bool mineBlock(std::unique_ptr<Dogero::IWalletLegacy>& wallet);
      bool mineBlock();
      bool startMining(size_t threads);
      bool stopMining();

      bool getNodeTransactionPool(size_t nodeIndex, Dogero::INode& node, std::vector<std::unique_ptr<Dogero::ITransactionReader>>& txPool);

      bool waitDaemonsReady();
      bool waitDaemonReady(size_t nodeIndex);
      bool waitForPeerCount(Dogero::INode& node, size_t expectedPeerCount);
      bool waitForPoolSize(size_t nodeIndex, Dogero::INode& node, size_t expectedPoolSize,
        std::vector<std::unique_ptr<Dogero::ITransactionReader>>& txPool);

      bool prepareAndSubmitBlock(TestNode& node, Dogero::Block&& blockTemplate);
    ...
    private:
      ...
      std::unique_ptr<Dogero::INode> mainNode;
      std::unique_ptr<Dogero::IWalletLegacy> workingWallet;
