...
uint64_t test_generator::getAlreadyGeneratedCoins(const Dogero::Block& blk) const {
  Crypto::Hash blkHash;
  get_block_hash(blk, blkHash);
  return getAlreadyGeneratedCoins(blkHash);
}

void test_generator::addBlock(const Dogero::Block& blk, size_t tsxSize, uint64_t fee,
...
bool test_generator::constructBlock(Dogero::Block& blk, uint32_t height, const Crypto::Hash& previousBlockHash,
...
bool test_generator::constructBlock(Dogero::Block& blk, const Dogero::AccountBase& minerAcc, uint64_t timestamp) {
  std::vector<size_t> blockSizes;
  std::list<Dogero::Transaction> txList;
  return constructBlock(blk, 0, NULL_HASH, minerAcc, timestamp, 0, blockSizes, txList);
}

bool test_generator::constructBlock(Dogero::Block& blk, const Dogero::Block& blkPrev,
                                    const Dogero::AccountBase& minerAcc,
...
bool test_generator::constructBlockManuallyTx(Dogero::Block& blk, const Dogero::Block& prevBlock,
                                              const Dogero::AccountBase& minerAcc,
...
bool test_generator::constructMaxSizeBlock(Dogero::Block& blk, const Dogero::Block& blkPrev,
                                           const Dogero::AccountBase& minerAccount,
...
void fillNonce(Dogero::Block& blk, const difficulty_type& diffic) {
  blk.nonce = 0;
  Crypto::cn_context context;
...
bool constructMinerTxBySize(const Dogero::Currency& currency, Dogero::Transaction& baseTransaction, uint32_t height,
  ...
  if (!currency.constructMinerTx(height, Common::medianValue(blockSizes), alreadyGeneratedCoins, targetBlockSize,
      fee, minerAddress, baseTransaction, Dogero::BinaryArray(), 1)) {
    return false;
  }
