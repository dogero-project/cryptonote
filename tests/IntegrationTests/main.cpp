...
public:
  ...
  SimpleTest(const Dogero::Currency& currency, System::Dispatcher& system, const Tests::Common::BaseFunctionalTestsConfig& config) :
    BaseFunctionalTests(currency, system, config) {}

  class WaitForActualGrowObserver : public Dogero::IWalletLegacyObserver {
  ...
  class WaitForActualDwindleObserver : public Dogero::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForPendingGrowObserver : public Dogero::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForConfirmationObserver : public Dogero::IWalletLegacyObserver {
  ...
  class WaitForSendCompletedObserver : public Dogero::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_Sent;
    std::error_code& m_error;
    Dogero::TransactionId& m_transactionId;

  public:
    WaitForSendCompletedObserver(Tests::Common::Semaphore& Sent, Dogero::TransactionId& transactionId, std::error_code& error) : m_Sent(Sent), m_transactionId(transactionId), m_error(error) { }
  ...
  class WaitForExternalTransactionObserver : public Dogero::IWalletLegacyObserver {
  public:
    WaitForExternalTransactionObserver() { }
    std::promise<Dogero::TransactionId> promise;
  ...
  class WaitForTransactionUpdated : public Dogero::IWalletLegacyObserver {
  public:
    ...
    Dogero::TransactionId expectindTxId;
  };
  ...
  bool perform1() {
    ...
    std::unique_ptr<Dogero::INode> node1;
    std::unique_ptr<Dogero::INode> node2;

    nodeDaemons.front()->makeINode(node1);
    nodeDaemons.front()->makeINode(node2);

    std::unique_ptr<Dogero::IWalletLegacy> wallet1;
    std::unique_ptr<Dogero::IWalletLegacy> wallet2;
  ...
  class WaitForBlockchainHeightChangeObserver : public Dogero::INodeObserver {
  ...
  class CallbackHeightChangeObserver : public Dogero::INodeObserver {
  ...
  bool perform2(size_t blocksCount = 10)
  {
    ...
    mineBlock();
    LOG_TRACE("STEP 2 PASSED");
    std::unique_ptr<Dogero::INode> localNode;
    std::unique_ptr<Dogero::INode> remoteNode;
  ...
  bool perform4() {
    ...
    std::unique_ptr<Dogero::INode> hopNode;
    std::unique_ptr<Dogero::INode> localNode;
    std::unique_ptr<Dogero::INode> remoteNode;
  ...
  bool perform5() {
    ...
    std::unique_ptr<Dogero::INode> node1;
    std::unique_ptr<Dogero::INode> inprocNode;
...
void testMultiVersion(const Dogero::Currency& currency, System::Dispatcher& d, const Tests::Common::BaseFunctionalTestsConfig& config);
...
TEST_F(SimpleTestCase, TESTPOOLDELETION) {
  currency = Dogero::CurrencyBuilder(logger).testnet(true).mempoolTxLiveTime(60).currency();
