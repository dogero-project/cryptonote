...
class InProcessNode : public INode, public Dogero::IDogeroProtocolObserver, public Dogero::ICoreObserver {
public:
  InProcessNode(Dogero::ICore& core, Dogero::IDogeroProtocolQuery& protocol);
...
private:
  ...
  void getNewBlocksAsync(std::vector<Crypto::Hash>& knownBlockIds, std::vector<Dogero::block_complete_entry>& newBlocks, uint32_t& startHeight, const Callback& callback);
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<Dogero::block_complete_entry>& newBlocks, uint32_t& startHeight);
  ...
  std::error_code doGetRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount,
      std::vector<Dogero::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result);

  void relayTransactionAsync(const Dogero::Transaction& transaction, const Callback& callback);
  std::error_code doRelayTransaction(const Dogero::Transaction& transaction);
