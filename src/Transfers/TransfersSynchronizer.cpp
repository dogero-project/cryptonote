...
TransfersSyncronizer::TransfersSyncronizer(const Dogero::Currency& currency, IBlockchainSynchronizer& sync, INode& node) :
...
void TransfersSyncronizer::load(std::istream& is) {
  ...
  Dogero::BinaryInputStreamSerializer s(inputStream);
  uint32_t version = 0;
