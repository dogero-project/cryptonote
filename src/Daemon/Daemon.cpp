...
namespace
{
  const command_line::arg_descriptor<std::string> arg_config_file = {"config-file", "Specify configuration file", std::string(Dogero::DOGERO_NAME) + ".conf"};
...
void print_genesis_tx_hex() {
  ...
  Dogero::Transaction tx = Dogero::CurrencyBuilder(logger).generateGenesisTransaction();
  Dogero::BinaryArray txb = Dogero::toBinaryArray(tx);
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    bool r = command_line::handle_error_helper(desc_options, [&]()
    {
      ...
      {
        std::cout << Dogero::DOGERO_NAME << " v" << PROJECT_VERSION_LONG << ENDL << ENDL;
    ...
    logger(INFO) << Dogero::DOGERO_NAME << " v" << PROJECT_VERSION_LONG;
    ...
    Dogero::CurrencyBuilder currencyBuilder(logManager);
    currencyBuilder.testnet(testnet_mode);

    try {
      currencyBuilder.currency();
    } catch (std::exception&) {
      std::cout << "GENESIS_COINBASE_TX_HEX constant has an incorrect value. Please launch: " << Dogero::DOGERO_NAME << "d --" << arg_print_genesis_tx.name;
      return 1;
    }

    Dogero::Currency currency = currencyBuilder.currency();
    Dogero::core ccore(currency, nullptr, logManager);

    Dogero::Checkpoints checkpoints(logManager);
    for (const auto& cp : Dogero::CHECKPOINTS) {
    ...
    Dogero::DogeroProtocolHandler cprotocol(currency, dispatcher, ccore, nullptr, logManager);
    Dogero::NodeServer p2psrv(dispatcher, cprotocol, logManager);
    Dogero::RpcServer rpcServer(dispatcher, logManager, ccore, p2psrv, cprotocol);
...
bool command_line_preprocessor(const boost::program_options::variables_map &vm, LoggerRef &logger) {
  ...
  if (command_line::get_arg(vm, command_line::arg_version)) {
    std::cout << Dogero::DOGERO_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
    exit = true;
  }
