...
namespace {
  ...
  static bool print_as_json(const T& obj) {
    std::cout << Dogero::storeToJson(obj) << ENDL;
    return true;
  }
}


DaemonCommandsHandler::DaemonCommandsHandler(Dogero::core& core, Dogero::NodeServer& srv, Logging::LoggerManager& log) :
...
{
  std::stringstream ss;
  ss << Dogero::DOGERO_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
  ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::start_mining(const std::vector<std::string> &args) {
  ...
  Dogero::AccountPublicAddress adr;
