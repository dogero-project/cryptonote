...
class NodeRpcProxy : public Dogero::INode {
public:
  ...
  virtual bool addObserver(Dogero::INodeRpcProxyObserver* observer);
  virtual bool removeObserver(Dogero::INodeRpcProxyObserver* observer);
...
private:
  ...
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>& knownBlockIds,
    std::vector<Dogero::block_complete_entry>& newBlocks, uint32_t& startHeight);
...
private:
  ...
  Tools::ObserverManager<Dogero::INodeObserver> m_observerManager;
  Tools::ObserverManager<Dogero::INodeRpcProxyObserver> m_rpcProxyObserverManager;
