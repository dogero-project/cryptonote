...
void JsonRpcServer::processRequest(const Dogero::HttpRequest& req, Dogero::HttpResponse& resp) {
  try {
    ...
    if (req.getUrl() == "/json_rpc") {
      ...
      try {
        ...
        resp.setStatus(Dogero::HttpResponse::STATUS_200);
      ...
      resp.setStatus(Dogero::HttpResponse::STATUS_404);
      return;
    }
  } catch (std::exception& e) {
    logger(Logging::WARNING) << "Error while processing http request: " << e.what();
    resp.setStatus(Dogero::HttpResponse::STATUS_500);
  }
}
