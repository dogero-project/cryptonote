...
class PaymentServiceJsonRpcServer : public Dogero::JsonRpcServer {
public:
...
private:
  ...
  HandlerFunction jsonHandler(RequestHandler handler) {
    return [handler] (const Common::JsonValue& jsonRpcParams, Common::JsonValue& jsonResponse) mutable {
      ...
      try {
        Dogero::JsonInputValueSerializer inputSerializer(const_cast<Common::JsonValue&>(jsonRpcParams));
      ...
      Dogero::JsonOutputStreamSerializer outputSerializer;
