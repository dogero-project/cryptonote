...
class WalletService {
public:
  WalletService(const Dogero::Currency& currency, System::Dispatcher& sys, Dogero::INode& node, Dogero::IWallet& wallet, const WalletConfiguration& conf, Logging::ILogger& logger);
...
private:
  ...
  std::vector<Dogero::TransactionsInBlockInfo> getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const;
  std::vector<Dogero::TransactionsInBlockInfo> getTransactions(uint32_t firstBlockIndex, size_t blockCount) const;
