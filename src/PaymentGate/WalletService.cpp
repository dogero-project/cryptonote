...
Crypto::Hash parsePaymentId(const std::string& paymentIdStr) {
  if (!checkPaymentId(paymentIdStr)) {
    throw std::system_error(make_error_code(Dogero::error::WalletServiceErrorCode::WRONG_PAYMENT_ID_FORMAT));
  }
...
bool getPaymentIdFromExtra(const std::string& binaryString, Crypto::Hash& paymentId) {
  return Dogero::getPaymentIdFromTxExtra(Common::asBinaryArray(binaryString), paymentId);
}
...
struct TransactionsInBlockInfoFilter {
  ...
  bool checkTransaction(const Dogero::WalletTransactionWithTransfers& transaction) const {
    if (havePaymentId) {
    ...
    for (const Dogero::WalletTransfer& transfer: transaction.transfers) {
...
void addPaymentIdToExtra(const std::string& paymentId, std::string& extra) {
  ...
  if (!Dogero::createTxExtraWithPaymentId(paymentId, extraVector)) {
...
Crypto::Hash parseHash(const std::string& hashString, Logging::LoggerRef logger) {
  ...
  if (!Common::podFromHex(hashString, hash)) {
    ...
    throw std::system_error(make_error_code(Dogero::error::WalletServiceErrorCode::WRONG_HASH_FORMAT));
  }

  return hash;
}

std::vector<Dogero::TransactionsInBlockInfo> filterTransactions(
  const std::vector<Dogero::TransactionsInBlockInfo>& blocks,
  const TransactionsInBlockInfoFilter& filter) {

  std::vector<Dogero::TransactionsInBlockInfo> result;

  for (const auto& block: blocks) {
    Dogero::TransactionsInBlockInfo item;
    item.blockHash = block.blockHash;

    for (const auto& transaction: block.transactions) {
      if (transaction.transaction.state != Dogero::WalletTransactionState::DELETED && filter.checkTransaction(transaction)) {
...
PaymentService::TransactionRpcInfo convertTransactionWithTransfersToTransactionRpcInfo(
  const Dogero::WalletTransactionWithTransfers& transactionWithTransfers) {
  ...
  for (const Dogero::WalletTransfer& transfer: transactionWithTransfers.transfers) {
...
std::vector<PaymentService::TransactionsInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionsInBlockRpcInfo(
  ...
  for (const auto& block: blocks) {
    ...
    for (const Dogero::WalletTransactionWithTransfers& transactionWithTransfers: block.transactions) {
...
std::vector<PaymentService::TransactionHashesInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(
  ...
  for (const Dogero::TransactionsInBlockInfo& block: blocks) {
    PaymentService::TransactionHashesInBlockRpcInfo item;
    item.blockHash = Common::podToHex(block.blockHash);

    for (const Dogero::WalletTransactionWithTransfers& transaction: block.transactions) {
...
void validateAddresses(const std::vector<std::string>& addresses, const Dogero::Currency& currency, Logging::LoggerRef logger) {
...
std::vector<Dogero::WalletOrder> convertWalletRpcOrdersToWalletOrders(const std::vector<PaymentService::WalletRpcOrder>& orders) {
  std::vector<Dogero::WalletOrder> result;
  result.reserve(orders.size());

  for (const auto& order: orders) {
    result.emplace_back(Dogero::WalletOrder {order.address, order.amount});
  }
...
void saveWallet(Dogero::IWallet& wallet, std::fstream& walletFile, bool saveDetailed = true, bool saveCache = true) {
  wallet.save(walletFile, saveDetailed, saveCache);
  walletFile.flush();
}

void secureSaveWallet(Dogero::IWallet& wallet, const std::string& path, bool saveDetailed = true, bool saveCache = true) {
...
void generateNewWallet(const Dogero::Currency &currency, const WalletConfiguration &conf, Logging::ILogger& logger, System::Dispatcher& dispatcher) {
  Logging::LoggerRef log(logger, "generateNewWallet");

  Dogero::INode* nodeStub = NodeFactory::createNodeStub();
  std::unique_ptr<Dogero::INode> nodeGuard(nodeStub);

  Dogero::IWallet* wallet = WalletFactory::createWallet(currency, *nodeStub, dispatcher);
  std::unique_ptr<Dogero::IWallet> walletGuard(wallet);
...
void importLegacyKeys(const std::string &legacyKeysFile, const WalletConfiguration &conf) {
  ...
  Dogero::importLegacyKeys(legacyKeysFile, conf.walletPassword, archive);
...
WalletService::WalletService(const Dogero::Currency& currency, System::Dispatcher& sys, Dogero::INode& node,
  Dogero::IWallet& wallet, const WalletConfiguration& conf, Logging::ILogger& logger) :
    currency(currency),
    wallet(wallet),
    node(node),
    config(conf),
...
std::error_code WalletService::resetWallet() {
  try {
    ...
    if (!inited) {
      ...
      return make_error_code(Dogero::error::NOT_INITIALIZED);
    }
    ...
    logger(Logging::WARNING) << "Error while reseting wallet: " << x.what();
    return make_error_code(Dogero::error::INTERNAL_WALLET_ERROR);
  }
...
std::error_code WalletService::replaceWithNewWallet(const std::string& viewSecretKeyText) {
  try {
    ...
    if (!Common::podFromHex(viewSecretKeyText, viewSecretKey)) {
      ...
      return make_error_code(Dogero::error::WalletServiceErrorCode::WRONG_KEY_FORMAT);
    }
...
std::error_code WalletService::getSpendkeys(const std::string& address, std::string& publicSpendKeyText, std::string& secretSpendKeyText) {
  try {
    ...
    Dogero::KeyPair key = wallet.getAddressSpendKey(address);
...
std::error_code WalletService::getViewKey(std::string& viewSecretKey) {
  try {
    System::EventLock lk(readyEvent);
    Dogero::KeyPair viewKey = wallet.getViewKey();
...
std::error_code WalletService::getTransaction(const std::string& transactionHash, TransactionRpcInfo& transaction) {
  try {
    ...
    Dogero::WalletTransactionWithTransfers transactionWithTransfers = wallet.getTransaction(hash);

    if (transactionWithTransfers.transaction.state == Dogero::WalletTransactionState::DELETED) {
      logger(Logging::WARNING) << "Transaction " << transactionHash << " is deleted";
      return make_error_code(Dogero::error::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::sendTransaction(const SendTransaction::Request& request, std::string& transactionHash) {
  try {
    ...
    Dogero::TransactionParameters sendParams;
...
std::error_code WalletService::deleteDelayedTransaction(const std::string& transactionHash) {
  try {
    ...
    if (idIt == transactionIdIndex.end()) {
      return make_error_code(Dogero::error::WalletServiceErrorCode::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::getUnconfirmedTransactionHashes(const std::vector<std::string>& addresses, std::vector<std::string>& transactionHashes) {
  try {
    ...
    std::vector<Dogero::WalletTransactionWithTransfers> transactions = wallet.getUnconfirmedTransactions();
...
void WalletService::refresh() {
  try {
    ...
    for (;;) {
      ...
      if (event.type == Dogero::TRANSACTION_CREATED) {
...
std::vector<Dogero::TransactionsInBlockInfo> WalletService::getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const {
  std::vector<Dogero::TransactionsInBlockInfo> result = wallet.getTransactions(blockHash, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(Dogero::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }

  return result;
}

std::vector<Dogero::TransactionsInBlockInfo> WalletService::getTransactions(uint32_t firstBlockIndex, size_t blockCount) const {
  std::vector<Dogero::TransactionsInBlockInfo> result = wallet.getTransactions(firstBlockIndex, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(Dogero::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(const Crypto::Hash& blockHash, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<Dogero::TransactionsInBlockInfo> allTransactions = getTransactions(blockHash, blockCount);
  std::vector<Dogero::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(uint32_t firstBlockIndex, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<Dogero::TransactionsInBlockInfo> allTransactions = getTransactions(firstBlockIndex, blockCount);
  std::vector<Dogero::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
  return convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(filteredTransactions);
}
