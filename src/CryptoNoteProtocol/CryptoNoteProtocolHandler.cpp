...
#include "DogeroProtocolHandler.h"
...
bool post_notify(IP2pEndpoint& p2p, typename t_parametr::request& arg, const DogeroConnectionContext& context) {
...
DogeroProtocolHandler::DogeroProtocolHandler(const Currency& currency, System::Dispatcher& dispatcher, ICore& rcore, IP2pEndpoint* p_net_layout, Logging::ILogger& log) :
...
size_t DogeroProtocolHandler::getPeerCount() const {
  return m_peersCount;
}

void DogeroProtocolHandler::set_p2p_endpoint(IP2pEndpoint* p2p) {
  if (p2p)
    m_p2p = p2p;
  else
    m_p2p = &m_p2p_stub;
...
void DogeroProtocolHandler::onConnectionOpened(DogeroConnectionContext& context) {
}

void DogeroProtocolHandler::onConnectionClosed(DogeroConnectionContext& context) {
  bool updated = false;
  {
  ...
  if (updated) {
    ...
    m_observerManager.notify(&IDogeroProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
  }

  if (context.m_state != DogeroConnectionContext::state_befor_handshake) {
    m_peersCount--;
    m_observerManager.notify(&IDogeroProtocolObserver::peerCountUpdated, m_peersCount.load());
  }
}

void DogeroProtocolHandler::stop() {
  m_stop = true;
}

bool DogeroProtocolHandler::start_sync(DogeroConnectionContext& context) {
  logger(Logging::TRACE) << context << "Starting synchronization";

  if (context.m_state == DogeroConnectionContext::state_synchronizing) {
...
bool DogeroProtocolHandler::get_stat_info(core_stat_info& stat_inf) {
  return m_core.get_stat_info(stat_inf);
}

void DogeroProtocolHandler::log_connections() {
  std::stringstream ss;
  ...
  m_p2p->for_each_connection([&](const DogeroConnectionContext& cntxt, PeerIdType peer_id) {
...
uint32_t DogeroProtocolHandler::get_current_blockchain_height() {
  uint32_t height;
  Crypto::Hash blockId;
...
bool DogeroProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, DogeroConnectionContext& context, bool is_inital) {
  if (context.m_state == DogeroConnectionContext::state_befor_handshake && !is_inital)
    return true;
  ...
  if (context.m_state == DogeroConnectionContext::state_synchronizing) {
    ...
    if (is_inital) {
      ...
      context.m_state = DogeroConnectionContext::state_pool_sync_required;
    } else {
      context.m_state = DogeroConnectionContext::state_normal;
    }
  } else {
    ...
    context.m_state = DogeroConnectionContext::state_sync_required;
  }
...
bool DogeroProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA& hshd) {
...
int notifyAdaptor(const BinaryArray& reqBuf, DogeroConnectionContext& ctx, Handler handler) {
...
int DogeroProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, DogeroConnectionContext& ctx, bool& handled) {
  int ret = 0;
  handled = true;

  switch (command) {
    HANDLE_NOTIFY(NOTIFY_NEW_BLOCK, &DogeroProtocolHandler::handle_notify_new_block)
    HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS, &DogeroProtocolHandler::handle_notify_new_transactions)
    HANDLE_NOTIFY(NOTIFY_REQUEST_GET_OBJECTS, &DogeroProtocolHandler::handle_request_get_objects)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_GET_OBJECTS, &DogeroProtocolHandler::handle_response_get_objects)
    HANDLE_NOTIFY(NOTIFY_REQUEST_CHAIN, &DogeroProtocolHandler::handle_request_chain)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY, &DogeroProtocolHandler::handle_response_chain_entry)
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &DogeroProtocolHandler::handleRequestTxPool)
...
int DogeroProtocolHandler::handle_notify_new_block(int command, NOTIFY_NEW_BLOCK::request& arg, DogeroConnectionContext& context) {
  ...
  if (context.m_state != DogeroConnectionContext::state_normal) {
    return 1;
  }

  for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
    Dogero::tx_verification_context tvc = boost::value_initialized<decltype(tvc)>();
    ...
    if (tvc.m_verifivation_failed) {
      ...
      context.m_state = DogeroConnectionContext::state_shutdown;
      return 1;
    }
  }
  ...
  if (bvc.m_added_to_main_chain) {
    ...
    context.m_state = DogeroConnectionContext::state_synchronizing;
    NOTIFY_REQUEST_CHAIN::request r = boost::value_initialized<NOTIFY_REQUEST_CHAIN::request>();
    r.block_ids = m_core.buildSparseChain();
    logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << r.block_ids.size();
    post_notify<NOTIFY_REQUEST_CHAIN>(*m_p2p, r, context);
...
int DogeroProtocolHandler::handle_notify_new_transactions(int command, NOTIFY_NEW_TRANSACTIONS::request& arg, DogeroConnectionContext& context) {
...
int DogeroProtocolHandler::handle_request_get_objects(int command, NOTIFY_REQUEST_GET_OBJECTS::request& arg, DogeroConnectionContext& context) {
...
int DogeroProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, DogeroConnectionContext& context) {
  ...
  for (const block_complete_entry& block_entry : arg.blocks) {
    ...
    if (count == 2) {
      if (m_core.have_block(get_block_hash(b))) {
        context.m_state = DogeroConnectionContext::state_idle;
  ...
  if (!m_stop && context.m_state == DogeroConnectionContext::state_synchronizing) {
...
int DogeroProtocolHandler::processObjects(DogeroConnectionContext& context, const std::vector<block_complete_entry>& blocks) {
...
bool DogeroProtocolHandler::on_idle() {
  return m_core.on_idle();
}

int DogeroProtocolHandler::handle_request_chain(int command, NOTIFY_REQUEST_CHAIN::request& arg, DogeroConnectionContext& context) {
...
bool DogeroProtocolHandler::request_missing_objects(DogeroConnectionContext& context, bool check_having_blocks) {
...
bool DogeroProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&IDogeroProtocolObserver::blockchainSynchronized, height);
  }
  return true;
}

int DogeroProtocolHandler::handle_response_chain_entry(int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request& arg, DogeroConnectionContext& context) {
...
int DogeroProtocolHandler::handleRequestTxPool(int command, NOTIFY_REQUEST_TX_POOL::request& arg,
                                                     DogeroConnectionContext& context) {
...
void DogeroProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_BLOCK::ID, buf);
}

void DogeroProtocolHandler::relay_transactions(NOTIFY_NEW_TRANSACTIONS::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_TRANSACTIONS::ID, buf);
}

void DogeroProtocolHandler::requestMissingPoolTransactions(const DogeroConnectionContext& context) {
...
void DogeroProtocolHandler::updateObservedHeight(uint32_t peerHeight, const DogeroConnectionContext& context) {
  bool updated = false;
  {
    std::lock_guard<std::mutex> lock(m_observedHeightMutex);
...
void DogeroProtocolHandler::recalculateMaxObservedHeight(const DogeroConnectionContext& context) {
  //should be locked outside
  uint32_t peerHeight = 0;
  m_p2p->for_each_connection([&peerHeight, &context](const DogeroConnectionContext& ctx, PeerIdType peerId) {
...
uint32_t DogeroProtocolHandler::getObservedHeight() const {
  std::lock_guard<std::mutex> lock(m_observedHeightMutex);
  return m_observedHeight;
};

bool DogeroProtocolHandler::addObserver(IDogeroProtocolObserver* observer) {
  return m_observerManager.add(observer);
}

bool DogeroProtocolHandler::removeObserver(IDogeroProtocolObserver* observer) {
