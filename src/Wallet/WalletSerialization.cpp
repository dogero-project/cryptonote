...
struct WalletTransactionDto {
  ...
  WalletTransactionDto(const Dogero::WalletTransaction& wallet) {
    state = wallet.state;
  ...
  Dogero::WalletTransactionState state;
  uint64_t timestamp;
  uint32_t blockHeight;
  Hash hash;
  int64_t totalAmount;
...
struct WalletTransferDto {
  ...
  WalletTransferDto(const Dogero::WalletTransfer& tr, uint32_t version) : WalletTransferDto(version) {
    address = tr.address;
    amount = tr.amount;
...
void serialize(WalletRecordDto& value, Dogero::ISerializer& serializer) {
...
void serialize(ObsoleteSpentOutputDto& value, Dogero::ISerializer& serializer) {
...
void serialize(ObsoleteChangeDto& value, Dogero::ISerializer& serializer) {
  serializer(value.txHash, "transaction_hash");
  serializer(value.amount, "amount");
}

void serialize(UnlockTransactionJobDto& value, Dogero::ISerializer& serializer) {
  serializer(value.blockHeight, "block_height");
  serializer(value.transactionHash, "transaction_hash");
  serializer(value.walletIndex, "wallet_index");
}

void serialize(WalletTransactionDto& value, Dogero::ISerializer& serializer) {
  typedef std::underlying_type<Dogero::WalletTransactionState>::type StateType;

  StateType state = static_cast<StateType>(value.state);
  serializer(state, "state");
  value.state = static_cast<Dogero::WalletTransactionState>(state);

  serializer(value.timestamp, "timestamp");
  Dogero::serializeBlockHeight(serializer, value.blockHeight, "block_height");
...
void serialize(WalletTransferDto& value, Dogero::ISerializer& serializer) {
...
std::string serialize(Object& obj, const std::string& name) {
  ...
  Dogero::BinaryOutputStreamSerializer s(output);
...
std::string encrypt(const std::string& plain, Dogero::CryptoContext& cryptoContext) {
  std::string cipher;
...
void addToStream(const std::string& cipher, const std::string& name, Common::IOutputStream& destination) {
  Dogero::BinaryOutputStreamSerializer s(destination);
  s(const_cast<std::string& >(cipher), name);
}

template<typename Object>
void serializeEncrypted(Object& obj, const std::string& name, Dogero::CryptoContext& cryptoContext, Common::IOutputStream& destination) {
...
std::string readCipher(Common::IInputStream& source, const std::string& name) {
  std::string cipher;
  Dogero::BinaryInputStreamSerializer s(source);
  s(cipher, name);

  return cipher;
}

std::string decrypt(const std::string& cipher, Dogero::CryptoContext& cryptoContext) {
  std::string plain;
...
template<typename Object>
void deserializeEncrypted(Object& obj, const std::string& name, Dogero::CryptoContext& cryptoContext, Common::IInputStream& source) {
...
void throwIfKeysMissmatch(const SecretKey& sec, const PublicKey& expected_pub) {
  if (!verifyKeys(sec, expected_pub))
    throw std::system_error(make_error_code(Dogero::error::WRONG_PASSWORD));
}

Dogero::WalletTransaction convert(const Dogero::WalletLegacyTransaction& tx) {
  Dogero::WalletTransaction mtx;

  mtx.state = Dogero::WalletTransactionState::SUCCEEDED;
...
Dogero::WalletTransfer convert(const Dogero::WalletLegacyTransfer& tr) {
  Dogero::WalletTransfer mtr;
...
void WalletSerializer::loadWallet(Common::IInputStream& source, const std::string& password, uint32_t version) {
  Dogero::CryptoContext cryptoContext;
...
void WalletSerializer::loadWalletV1(Common::IInputStream& source, const std::string& password) {
  ...
  Dogero::BinaryInputStreamSerializer encrypted(source);
  ...
  Dogero::BinaryInputStreamSerializer serializer(decryptedStream);
...
void WalletSerializer::loadWalletV1Keys(Dogero::BinaryInputStreamSerializer& serializer) {
  Dogero::KeysStorage keys;
...
void WalletSerializer::loadWalletV1Details(Dogero::BinaryInputStreamSerializer& serializer) {
...
void WalletSerializer::loadWallets(Common::IInputStream& source, CryptoContext& cryptoContext) {
  ...
  for (uint64_t i = 0; i < count; ++i) {
    ...
    wallet.container = reinterpret_cast<Dogero::ITransfersContainer*>(i); //dirty hack. container field must be unique
