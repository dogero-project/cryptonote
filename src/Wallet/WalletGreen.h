...
protected:
  ...
  struct ReceiverAmounts {
    Dogero::AccountPublicAddress receiver;
  ...
  void updateBalance(Dogero::ITransfersContainer* container);
  ...
  const WalletRecord& getWalletRecord(Dogero::ITransfersContainer* container) const;

  Dogero::AccountPublicAddress parseAddress(const std::string& address) const;
  ...
  void prepareTransaction(std::vector<WalletOuts>&& wallets,
    ...
    const Dogero::AccountPublicAddress& changeDestinationAddress,
  ...
  std::unique_ptr<Dogero::ITransaction> makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
    std::vector<InputInfo>& keysInfo, const std::string& extra, uint64_t unlockTimestamp);

  void sendTransaction(const Dogero::Transaction& cryptoNoteTransaction);
  ...
  bool updateWalletTransactionInfo(size_t transactionId, const Dogero::TransactionInformation& info, int64_t totalAmount);
  ...
  void insertUnlockTransactionJob(const Crypto::Hash& transactionHash, uint32_t blockHeight, Dogero::ITransfersContainer* container);
  ...
  Dogero::AccountPublicAddress getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const;
