...
void parseAddressString(const std::string& string, const Dogero::Currency& currency, Dogero::AccountPublicAddress& address) {
  if (!currency.parseAccountAddressString(string, address)) {
    throw std::system_error(make_error_code(Dogero::error::BAD_ADDRESS));
  }
}

void validateAddresses(const std::vector<std::string>& addresses, const Dogero::Currency& currency) {
  for (const auto& address: addresses) {
    if (!Dogero::validateAddress(address, currency)) {
      throw std::system_error(make_error_code(Dogero::error::BAD_ADDRESS));
    }
  }
}

void validateOrders(const std::vector<WalletOrder>& orders, const Dogero::Currency& currency) {
  for (const auto& order: orders) {
    if (!Dogero::validateAddress(order.address, currency)) {
      throw std::system_error(make_error_code(Dogero::error::BAD_ADDRESS));
    }

    if (order.amount >= static_cast<uint64_t>(std::numeric_limits<int64_t>::max())) {
      throw std::system_error(make_error_code(Dogero::error::WRONG_AMOUNT),
        "Order amount must not exceed " + std::to_string(std::numeric_limits<int64_t>::max()));
    }
  }
}

uint64_t countNeededMoney(const std::vector<Dogero::WalletTransfer>& destinations, uint64_t fee) {
  uint64_t neededMoney = 0;
  for (const auto& transfer: destinations) {
    if (transfer.amount == 0) {
      throw std::system_error(make_error_code(Dogero::error::ZERO_DESTINATION));
    ...
    if (neededMoney < uamount) {
      throw std::system_error(make_error_code(Dogero::error::SUM_OVERFLOW));
    }
  }
...
void checkIfEnoughMixins(std::vector<Dogero::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult, uint64_t mixIn) {
  auto notEnoughIt = std::find_if(mixinResult.begin(), mixinResult.end(),
    [mixIn] (const Dogero::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount& ofa) { return ofa.outs.size() < mixIn; } );

  if (mixIn == 0 && mixinResult.empty()) {
    throw std::system_error(make_error_code(Dogero::error::MIXIN_COUNT_TOO_BIG));
  }
...
Dogero::WalletEvent makeTransactionUpdatedEvent(size_t id) {
  Dogero::WalletEvent event;
  event.type = Dogero::WalletEventType::TRANSACTION_UPDATED;
  event.transactionUpdated.transactionIndex = id;

  return event;
}

Dogero::WalletEvent makeTransactionCreatedEvent(size_t id) {
  Dogero::WalletEvent event;
  event.type = Dogero::WalletEventType::TRANSACTION_CREATED;
  event.transactionCreated.transactionIndex = id;

  return event;
}

Dogero::WalletEvent makeMoneyUnlockedEvent() {
  Dogero::WalletEvent event;
  event.type = Dogero::WalletEventType::BALANCE_UNLOCKED;

  return event;
}

Dogero::WalletEvent makeSyncProgressUpdatedEvent(uint32_t current, uint32_t total) {
  Dogero::WalletEvent event;
  event.type = Dogero::WalletEventType::SYNC_PROGRESS_UPDATED;
...
Dogero::WalletEvent makeSyncCompletedEvent() {
  Dogero::WalletEvent event;
  event.type = Dogero::WalletEventType::SYNC_COMPLETED;
...
Dogero::AccountPublicAddress parseAccountAddressString(const std::string& addressString, const Dogero::Currency& currency) {
  Dogero::AccountPublicAddress address;
...
void WalletGreen::initializeWithViewKey(const Crypto::SecretKey& viewSecretKey, const std::string& password) {
  ...
  if (!Crypto::secret_key_to_public_key(viewSecretKey, viewPublicKey)) {
    throw std::system_error(make_error_code(Dogero::error::KEY_GENERATION_ERROR));
  }
...
void WalletGreen::initWithKeys(const Crypto::PublicKey& viewPublicKey, const Crypto::SecretKey& viewSecretKey, const std::string& password) {
  if (m_state != WalletState::NOT_INITIALIZED) {
    throw std::system_error(make_error_code(Dogero::error::ALREADY_INITIALIZED));
  }
...
KeyPair WalletGreen::getAddressSpendKey(const std::string& address) const {
  ...
  Dogero::AccountPublicAddress pubAddr = parseAddress(address);
...
WalletTransaction WalletGreen::getTransaction(size_t transactionIndex) const {
  ...
  if (m_transactions.size() <= transactionIndex) {
    throw std::system_error(make_error_code(Dogero::error::INDEX_OUT_OF_RANGE));
  }
...
void WalletGreen::prepareTransaction(std::vector<WalletOuts>&& wallets,
  ...
  const Dogero::AccountPublicAddress& changeDestination,
  ...
  typedef Dogero::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount outs_for_amount;
...
void WalletGreen::validateTransactionParameters(const TransactionParameters& transactionParameters) {
  ...
  if (transactionParameters.changeDestination.empty()) {
    ...
    if (!Dogero::validateAddress(transactionParameters.changeDestination, m_currency)) {
      throw std::system_error(make_error_code(Dogero::error::BAD_ADDRESS), "Wrong change address");
    }
...
size_t WalletGreen::doTransfer(const TransactionParameters& transactionParameters) {
  ...
  Dogero::AccountPublicAddress changeDestination = getChangeDestination(transactionParameters.changeDestination, transactionParameters.sourceAddresses);
...
size_t WalletGreen::makeTransaction(const TransactionParameters& sendingTransaction) {
  ...
  Dogero::AccountPublicAddress changeDestination = getChangeDestination(sendingTransaction.changeDestination, sendingTransaction.sourceAddresses);
...
size_t WalletGreen::insertOutgoingTransactionAndPushEvent(const Hash& transactionHash, uint64_t fee, const BinaryArray& extra, uint64_t unlockTimestamp) {
  ...
  insertTx.blockHeight = Dogero::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT;
...
bool WalletGreen::updateWalletTransactionInfo(size_t transactionId, const Dogero::TransactionInformation& info, int64_t totalAmount) {
...
std::unique_ptr<Dogero::ITransaction> WalletGreen::makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
...
void WalletGreen::sendTransaction(const Dogero::Transaction& cryptoNoteTransaction) {
  System::Event completion(m_dispatcher);
  std::error_code ec;
...
size_t WalletGreen::validateSaveAndSendTransaction(const ITransactionReader& transaction, const std::vector<WalletTransfer>& destinations, bool isFusion, bool send) {
  ...
  Dogero::Transaction cryptoNoteTransaction;
...
void WalletGreen::requestMixinOuts(
  ...
  uint64_t mixIn,
  std::vector<Dogero::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult) {
...
std::vector<Dogero::WalletGreen::ReceiverAmounts> WalletGreen::splitDestinations(const std::vector<Dogero::WalletTransfer>& destinations,
  uint64_t dustThreshold,
...
Dogero::WalletGreen::ReceiverAmounts WalletGreen::splitAmount(
  uint64_t amount,
...
void WalletGreen::prepareInputs(
  ...
  typedef Dogero::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::out_entry out_entry;
...
void WalletGreen::throwIfNotInitialized() const {
  if (m_state != WalletState::INITIALIZED) {
    throw std::system_error(make_error_code(Dogero::error::NOT_INITIALIZED));
  }
}
...
void WalletGreen::transactionUpdated(const TransactionInformation& transactionInfo, const std::vector<ContainerAmounts>& containerAmountsList) {
  ...
  if (transactionInfo.blockHeight != Dogero::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT) {
...
void WalletGreen::transactionDeleted(ITransfersSubscription* object, const Hash& transactionHash) {
  ...
  Dogero::ITransfersContainer* container = &object->getContainer();
  updateBalance(container);
  deleteUnlockTransactionJob(transactionHash);

  bool updated = false;
  m_transactions.get<TransactionIndex>().modify(it, [&updated](Dogero::WalletTransaction& tx) {
...
void WalletGreen::insertUnlockTransactionJob(const Hash& transactionHash, uint32_t blockHeight, Dogero::ITransfersContainer* container) {
  auto& index = m_unlockTransactionsJob.get<BlockHeightIndex>();
...
void WalletGreen::updateBalance(Dogero::ITransfersContainer* container) {
...
const WalletRecord& WalletGreen::getWalletRecord(Dogero::ITransfersContainer* container) const {
  auto it = m_walletsContainer.get<TransfersContainerIndex>().find(container);
  if (it == m_walletsContainer.get<TransfersContainerIndex>().end()) {
    throw std::system_error(make_error_code(error::WALLET_NOT_FOUND));
  }
...
Dogero::AccountPublicAddress WalletGreen::parseAddress(const std::string& address) const {
...
Dogero::AccountPublicAddress WalletGreen::getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const {
...
bool WalletGreen::isMyAddress(const std::string& addressString) const {
  Dogero::AccountPublicAddress address = parseAccountAddressString(addressString, m_currency);
